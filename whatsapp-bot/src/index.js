...
async function startBot() {
  ...
  const { state, saveCreds, clearState } = await useMongoAuthState(authCollection);
...
async function sendMessage({ to, message }) {
  if (connectionState !== 'open') {
    throw new Error('WhatsApp বট এখনো connected না — /qr পেজ থেকে QR স্ক্যান করুন।');
  }
...
app.get('/qr', requireSecret, async (req, res) => {
  ...
  if (!latestQR) {
    res.set('Refresh', '3');
    return res.send('<h2>QR তৈরি হচ্ছে… এই পেজটা কয়েক সেকেন্ড পর নিজে থেকেই রিফ্রেশ হবে।</h2>');
  }
...
app.post('/send', requireSecret, async (req, res) => {
  ...
  if (!to || !message) return res.status(400).json({ error: 'to এবং message দিন' });
  if (connectionState !== 'open') {
    return res.status(503).json({ error: 'WhatsApp বট এখনো connected না — /qr পেজ থেকে QR স্ক্যান করুন।' });
  }
