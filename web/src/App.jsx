...
import ReportPassModal, { isReportsUnlocked } from './components/ReportPassModal.jsx';
    ...
    <DataProvider>
      <div className="app">
        ...
        <main className="main">
          ...
          <footer
            className="no-print"
            style={{ marginTop: 18, padding: 12, textAlign: 'center', color: '#667085', fontSize: 12, borderTop: '1px solid #e7ebf0' }}
          >
            © 2025 শিশু কানন মডেল একাডেমি | মিরওয়ারিশপুর, বেগমগঞ্জ, নোয়াখালী | Developed with ❤️ for Shishu Kanon
          </footer>
        </main>
      </div>
      <div className="mobile-nav">
        {MOBILE_NAV_ITEMS.map(([id, icon, label]) => (
          <button key={id} onClick={() => navigate(id)}>{icon}<br />{label}</button>
        ))}
      </div>
      {showReportGate && (
        <ReportPassModal
          onClose={() => setShowReportGate(false)}
          onUnlock={() => { setReportsUnlocked(true); setShowReportGate(false); setPage('reports'); }}
        />
      )}
    </DataProvider>
  );
  ...
  if (!admin) {
    return (
      <>
        ...
        {showAdminReg && <AdminRegModal onClose={() => setShowAdminReg(false)} />}
      </>
    );
  }
  return <AppShell />;
