...
export function formatHeaderDate(now = new Date()) {
  return new Intl.DateTimeFormat('bn-BD', { year: 'numeric', month: 'long', day: 'numeric' }).format(now);
}
export function formatHeaderTime(now = new Date()) {
  return new Intl.DateTimeFormat('bn-BD', {
    weekday: 'long', hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true,
  }).format(now);
}
...
export function salaryIsType(x, type) {
  return x.staffType === type || (!x.staffType && String(x.teacherId || '').startsWith(type === 'teacher' ? 'TC-' : 'EMP-'));
}
...
export function exportAllExcel(data) {
  const XLSX = getXLSX();
  if (!XLSX) { alert('Excel library load হয়নি। Internet connection চালু করে আবার চেষ্টা করুন।'); return; }
  const wb = XLSX.utils.book_new();
  const sheets = {
    'ভর্তি ও ছাত্র-ছাত্রী': (data.Students || []).map((s) => ({
      Student_ID: s.id, নাম_বাংলা: s.nameBn || s.name, নাম_English: s.nameEn || '', লিঙ্গ: s.gender,
      জন্মতারিখ: s.dob, শ্রেণী: s.class, Blood_Group: s.blood, পিতার_নাম: s.father, মাতার_নাম: s.mother,
      অভিভাবক: s.guardian, Guardian_Mobile: s.guardianMobile, Guardian_NID: s.guardianNid,
      আয়ের_উৎস: s.incomeSource, মাসিক_বেতন_দিতে_পারবেন: s.canPay, ঠিকানা: s.address,
      ভর্তি_তারিখ: s.admissionDate, ভর্তি_ফি: s.admissionFee, পুনঃভর্তি_ফি: s.renewFee,
      ফরম_ফি: s.formFee, বকেয়া: s.due, Verified: s.verified ? 'হ্যাঁ' : 'না',
    })),
    'Fee Collection': data.Fees || [],
    Attendance: data.Attendance || [],
    'Teachers Employees': (data.Teachers || []).map((t) => ({
      ID: t.id, নাম: t.name, পদবি: t.role, মোবাইল: t.mobile, ঠিকানা: t.address,
      যোগদানের_তারিখ: t.joinDate, বেতন: t.salary,
    })),
    'Teacher Salary': (data.Salaries || []).filter((x) => salaryIsType(x, 'teacher')),
    'Employee Salary': (data.Salaries || []).filter((x) => salaryIsType(x, 'employee')),
...
export function reportStudents(students) {
  const rows = (students || []).map((s) => (
    `<tr><td>${s.id}</td><td>${s.nameBn || s.name || ''}<br>${s.nameEn || ''}</td><td>${s.father || ''}</td>`
    + `<td>${s.mother || ''}</td><td>${s.class}</td><td>${s.guardianMobile || ''}</td><td>${money(s.due)}</td></tr>`
  )).join('');
  openPrintWindow(
...
export function reportFees(fees) {
  ...
  const rows = (fees || []).map((x) => (
    `<tr><td>${x.receipt}</td><td>${x.studentName || x.studentId}</td><td>${x.date || ''}</td>`
    + `<td>${x.method}</td><td>${money(x.total)}</td></tr>`
  )).join('');
  openPrintWindow(
    `<html><body><h2>Fee Report — শিশু কানন মডেল একাডেমি</h2><p>সর্বমোট গ্রহণ: <b>${money(total)}</b></p>`
...
export function reportFinance({ Fees, Salaries, Expenses }) {
  ...
  const teacherSalary = (Salaries || []).filter((x) => salaryIsType(x, 'teacher')).reduce((a, x) => a + Number(x.amount || 0), 0);
  const employeeSalary = (Salaries || []).filter((x) => salaryIsType(x, 'employee')).reduce((a, x) => a + Number(x.amount || 0), 0);
  const expense = (Expenses || []).reduce((a, x) => a + Number(x.amount || 0), 0);
  const salary = teacherSalary + employeeSalary;
  openPrintWindow(
    `<html><body><h2>Financial Report — শিশু কানন মডেল একাডেমি</h2><p>Fee Income: <b>${money(income)}</b></p>`
    + `<p>শিক্ষকের বেতন: <b>${money(teacherSalary)}</b></p><p>কর্মচারীর বেতন: <b>${money(employeeSalary)}</b></p>`
    + `<p>অন্যান্য খরচ: <b>${money(expense)}</b></p><p>Net: <b>${money(income - salary - expense)}</b></p>`
    + '<script>window.print()<\/script></body></html>'
  );
}
